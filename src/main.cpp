...
void photo_save(const char * fileName, const bool sd_sign) {
  ...
  if (fb) esp_camera_fb_return(fb);      // return dummy
  ...
  if (!fb) {
    ...
    turnOffLedRing();  // Always turn off even on failure
    return;
  }
  ...
  delay(1000);
  turnOffLedRing();  // Turn off after capture
}
...
void loop() {
  ...
  // Camera & SD available, take a picture
  if(camera_sign){
    char filename[32];
